...
function getUniekeOpleidingen() {
  ...
  return [
    ...new Set(
      studenten
        .map((s) =>
          s && s.opleiding_naam ? String(s.opleiding_naam).trim() : null
        )
        .filter(Boolean)
    ),
  ];
  ...
  const alleFuncties = studenten.flatMap((s) => {
    ...
    if (Array.isArray(s.functies)) {
      return s.functies.map((f) =>
        typeof f === 'string' ? f.trim() : f && f.naam ? f.naam.trim() : null
      );
    }
...
function getUniekeStudiejaren() {
  ...
  return [
    ...new Set(
      studenten
        .map((s) => (s && s.studiejaar ? String(s.studiejaar).trim() : null))
        .filter(Boolean)
    ),
  ];
}
...
function filterStudenten({
  ...
  let filtered = studenten.filter((s) => {
    // Veilige string concatenatie met null checks
    const searchString = `${s.voornaam || ''} ${s.achternaam || ''} ${
      s.contact_email || ''
    }`;
    const matchZoek = zoek
      ? searchString.toLowerCase().includes(zoek.toLowerCase())
      : true;

    const matchOpleiding = opleidingen.length
      ? opleidingen.includes(s.opleiding_naam)
      : true;

    // Fix voor skills filtering - alleen filteren als er skills geselecteerd zijn
    const matchSkill =
      skills.length > 0
    ...
    // Fix voor functies filtering - alleen filteren als er functies geselecteerd zijn
    const matchFunctie =
      functies.length > 0
        ? Array.isArray(s.functies) &&
          s.functies.some((f) => functies.includes(f.naam || f))
        : true;

    const matchStudiejaar = studiejaren.length
      ? studiejaren.includes(s.studiejaar)
      : true;
    return (
      matchZoek &&
      matchOpleiding &&
...
async function showStudentPopup(student) {
  ...
  const { skills: studentSkills, functies: studentFuncties } =
    await fetchStudentSkillsAndFuncties(student.gebruiker_id);
  ...
  try {
    ...
    opleidingText =
      (opleidingResponse.type ? opleidingResponse.type + ' ' : '') +
      (opleidingResponse.naam || 'Onbekend');
  } catch (e) {
  ...
  try {
    ...
    const [
      ...
      authenticatedFetch(
        `https://api.ehb-match.me/speeddates/accepted?id=${student.gebruiker_id}`
      ).then((r) => (r.ok ? r.json() : [])),
      authenticatedFetch(
        `https://api.ehb-match.me/speeddates/accepted?id=${currentBedrijfId}`
      ).then((r) => (r.ok ? r.json() : [])),
      authenticatedFetch(
        `https://api.ehb-match.me/speeddates/pending?id=${student.gebruiker_id}`,
        { headers: { Authorization: `Bearer ${token}` } }
      ).then((r) => (r.ok ? r.json() : [])),
      authenticatedFetch(
        `https://api.ehb-match.me/speeddates/pending?id=${currentBedrijfId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      ).then((r) => (r.ok ? r.json() : [])),
    ]);
  ...
  try {
    const resp = await authenticatedFetch(
      `https://api.ehb-match.me/bedrijven/${currentBedrijfId}/skills`
    );
    if (resp.ok) bedrijfSkills = await resp.json();
  } catch {}
  try {
    const resp = await authenticatedFetch(
      `https://api.ehb-match.me/bedrijven/${currentBedrijfId}/functies`
    );
  ...
  const skillsHTML =
    Array.isArray(studentSkills) && studentSkills.length
      ? studentSkills
          .map(
            (s) =>
              `<span class="skill-badge" style="display:inline-block;margin:0 0.3em 0.3em 0;padding:0.2em 0.7em;border-radius:7px;background:${
                bedrijfSkillNamen.includes(s) ? '#e3f2fd' : '#f5f5f5'
              };color:#222;font-size:0.97em;" title="${
                bedrijfSkillNamen.includes(s)
                  ? 'Deze skill matcht met het bedrijf!'
                  : ''
              }">${s}</span>`
          )
          .join(' ')
  ...
  const functiesHTML =
    Array.isArray(studentFuncties) && studentFuncties.length
      ? studentFuncties
          .map(
            (f) =>
              `<span class="functie-badge" style="display:inline-block;margin:0 0.3em 0.3em 0;padding:0.2em 0.7em;border-radius:7px;background:${
                bedrijfFunctieNamen.includes(f) ? '#e3f2fd' : '#f5f5f5'
              };color:#222;font-size:0.97em;" title="${
                bedrijfFunctieNamen.includes(f)
                  ? 'Deze functie matcht met het bedrijf!'
                  : ''
              }">${f}</span>`
          )
          .join(' ')
  ...
  const eventsResp = await authenticatedFetch(
    `https://api.ehb-match.me/bedrijven/${currentBedrijfId}/events`
  );
  ...
  popup.innerHTML = `
    <div id="student-popup-content" style="background:#fff;padding:2.2rem 2rem 1.5rem 2rem;border-radius:16px;box-shadow:0 8px 32px rgba(0,0,0,0.18);max-width:600px;width:98vw;min-width:340px;position:relative;display:flex;flex-direction:column;align-items:center;">
      ...
      }" style="color:#444;">${student.contact_email}</a></div>
  ...
  if (datePicker) {
    datePicker.addEventListener('change', (e) => {
      ...
      urenLijst.innerHTML = '';
      uren.forEach((uur) => {
        const slotsForHour = allSlots.filter(
          (slot) => parseInt(slot.value.split(':')[0], 10) === uur
        );
        ...
        btn.style.cssText = `background:${backgroundColor};border:1.5px solid ${borderColor};border-radius:8px;padding:0.5rem 1.2rem;font-size:1.05rem;cursor:${
          isUnavailable ? 'not-allowed' : 'pointer'
        };margin:0;transition:box-shadow .2s;display:flex;align-items:center;gap:0.4em;color:${textColor};opacity:${
          isUnavailable ? '0.7' : '1'
        };`;
        if (isUnavailable) {
          btn.disabled = true;
  ...
  function renderSlotsForHour(uur) {
    ...
    slotsList.innerHTML = slotsForHour
      .map(
        ...
        };font-size:0.97rem;" ${slot.disabled ? 'disabled' : ''} title="${
          slot.label
  ...
  aanvraagBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      const response = await createSpeeddate(
        student.gebruiker_id,
        currentBedrijfId,
        datetime
      );
      ...
      if (gekozenTijd) {
        const slotBtn = slotsList.querySelector(
          `.slot-btn[data-slot="${gekozenTijd}"]`
        );
        if (slotBtn) {
...
export async function showStudentInfoPopup(student) {
  ...
  const naam =
    (studentData.voornaam || '') + ' ' + (studentData.achternaam || '');
  ...
  const profielFoto = getStudentPhotoUrl(
    studentData.profiel_foto_key,
    studentData.profiel_foto_url
  );

  // Toon alleen niet-lege velden
  const opleidingHtml = opleiding
    ? `<div style="font-size:1rem;color:#666;margin-bottom:0.3rem;">${opleiding}</div>`
    : '';
  const studiejaarHtml = studiejaar
    ? `<div style=\"font-size:0.97rem;color:#888;margin-bottom:0.7rem;\">Studiejaar: ${studiejaar}</div>`
    : '';
  const emailHtml = email
    ? `<div style=\"font-size:0.95rem;color:#555;text-align:center;margin-bottom:0.5rem;\"><a href=\"mailto:${email}\" style=\"color:#444;\">${email}</a></div>`
    : '';
          ...
          <div style=\"margin-top:0.3rem;max-width:100%;white-space:normal;display:flex;flex-wrap:wrap;gap:0.3em;\">
            ${
              Array.isArray(functies) && functies.length
                ? functies
                    .map((f) => {
                      const isMatch = bedrijfFuncties.includes(
                        (f.naam || '').toLowerCase()
                      );
          ...
          <div style=\"margin-top:0.3rem;max-width:100%;white-space:normal;display:flex;flex-wrap:wrap;gap:0.3em;\">
            ${
              Array.isArray(skills) && skills.length
                ? skills
                    .map((s) => {
                      const isMatch = bedrijfSkills.includes(
                        (s.naam || '').toLowerCase()
                      );
  ...
  function renderStudentsList(studentsToShow = studenten) {
    ...
    listElement.innerHTML = studentsToShow
      .map((student, idx) => {
        ...
        const studiejaar = student.studiejaar
          ? `Studiejaar: ${student.studiejaar}`
          : '';
        ...
        return `
          <div class="student-card" data-student-idx="${idx}" style="background:#fff;border-radius:12px;box-shadow:0 2px 8px #0001, -8px 0 16px 0 ${
          colorScheme.background
        }33;padding:1.5rem 1rem;display:flex;flex-direction:column;align-items:center;width:220px;cursor:pointer;transition:box-shadow 0.2s;position:relative;">
            <span class="match-badge" style="position:absolute;top:10px;left:10px;background:${
              colorScheme.background
            };color:#fff;font-weight:bold;padding:0.3em 0.8em;border-radius:16px;font-size:0.98em;z-index:3;box-shadow:0 2px 8px #0002;min-width:unset;max-width:70px;text-align:center;">${formattedMatch}%</span>
      ...
      .join('');
    document
      .querySelectorAll('.student-card .student-card-clickable')
      .forEach((card) => {
...
`;
  ...
  function renderFilterOptions() {
    // Opleidingen
    const opleidingen = getUniekeOpleidingen().sort((a, b) =>
      a.localeCompare(b)
    );
    ...
    // Studiejaar
    const studiejaren = getUniekeStudiejaren().sort((a, b) =>
      a.localeCompare(b)
    );
  ...
  function createPopup({
    ...
    function renderOptions(filter = '') {
      ...
      if (filtered.length === 0) {
        optionsContainer.innerHTML =
          '<div style="color:#888;padding:1rem 0;">Geen opties beschikbaar.</div>';
      } else {
    ...
    try {
      ...
      await Promise.all(
        studenten.map(async (s) => {
          try {
            const skillsResp = await authenticatedFetch(
              `https://api.ehb-match.me/studenten/${s.gebruiker_id}/skills`
            );
            s.skills = skillsResp.ok ? await skillsResp.json() : [];
            const functiesResp = await authenticatedFetch(
              `https://api.ehb-match.me/studenten/${s.gebruiker_id}/functies`
            );
  ...
  setupFilterbarEvents(); // Setup event listeners voor zoekbalk en filters
}
