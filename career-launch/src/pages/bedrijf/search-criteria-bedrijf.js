...
export function renderSearchCriteriaBedrijf(rootElement, bedrijfData = {}) {
  ...
  function renderSelectedLanguages() {
    ...
    const languagesHtml = selectedLanguages
      .map(
        (language) => `
      <div class="skill-tag">
    ...
    // Add event listeners for remove buttons
    document
      .querySelectorAll('#selected-languages .remove-skill')
      .forEach((btn) => {
        btn.addEventListener('click', async (e) => {
          ...
          if (
            selectedLanguages.some(
              (language) => language.naam === languageToRemove
            )
          ) {
            button.textContent = originalText;
            button.disabled = false;
  ...
  async function addLanguage() {
    ...
    try {
      ...
      let existingLanguage = availableLanguages.find(
        (language) =>
          language.naam.toLowerCase() === languageValue.toLowerCase()
      );
      ...
      if (existingLanguage) {
        ...
        if (!languageToAdd) {
          alert(
            'Taal bestaat mogelijk al of er is een probleem opgetreden. Probeer de pagina te vernieuwen.'
          );
          return;
        }

        // Controleer nogmaals of taal niet al is toegevoegd tijdens het aanmaken
        const duplicateCheck = availableLanguages.find(
          (language) => language.id === languageToAdd.id
        );
        if (!duplicateCheck) {
      ...
      if (!languageToAdd.id && languageToAdd.id !== 0) {
        alert(
          'Taal toegevoegd lokaal, maar kan niet worden opgeslagen (geen ID)'
        );
        return;
      }
      ...
      if (!success) {
        alert(
          'Taal is aangemaakt maar kon niet worden toegevoegd aan uw bedrijfsprofiel. Controleer uw internetverbinding en probeer het opnieuw.'
        );
  ...
  async function removeLanguage(languageToRemove) {
    ...
    const languageToDelete = selectedLanguages.find(
      (language) => language.naam === languageToRemove
    );
        ...
        if (Array.isArray(functies) && functies.length > 0) {
          ...
          werktypeNames.forEach((werktype) => {
            const checkbox = document.querySelector(
              `input[name="werktype"][value="${werktype}"]`
            );
            if (checkbox) {
  ...
  Promise.allSettled([
    ...
    const [
      ...
      languagesResult,
      bedrijfSkillsResult,
      bedrijfLanguagesResult,
      bedrijfWerktypesResult,
    ] = results;

    if (skillsResult.status === 'rejected') {
      console.warn(
        'Skills database kon niet geladen worden, maar fallback is beschikbaar'
      );
    }

    if (languagesResult.status === 'rejected') {
      console.warn(
        'Languages database kon niet geladen worden, maar fallback is beschikbaar'
      );
    }
