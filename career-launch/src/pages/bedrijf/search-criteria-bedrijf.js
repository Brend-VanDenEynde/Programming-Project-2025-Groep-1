...
async function fetchAlleFuncties() {
  try {
    ...
    // Fallback naar hardcoded functies
    return [
      { id: 1, naam: 'Fulltime' },
      { id: 2, naam: 'Parttime' },
      { id: 3, naam: 'Stage' },
...
// Globale variabele voor functies
let alleBeschikbareFuncties = [];

// Zoek-criteria opslaan en laden voor bedrijf
function saveSearchCriteria(bedrijfId, criteria) {
  const storageKey = `searchCriteria_bedrijf_${bedrijfId}`;
  try {
    sessionStorage.setItem(storageKey, JSON.stringify(criteria));
  } catch (error) {
    console.error('Fout bij opslaan zoek-criteria:', error);
  }
}

function loadSearchCriteria(bedrijfId) {
  const storageKey = `searchCriteria_bedrijf_${bedrijfId}`;
  try {
    const stored = sessionStorage.getItem(storageKey);
    return stored ? JSON.parse(stored) : null;
  } catch (error) {
    console.error('Fout bij laden zoek-criteria:', error);
    return null;
  }
}

function collectCurrentCriteria(bedrijfId) {
  const selectedFuncties = Array.from(
    document.querySelectorAll('input[name="bedrijfFuncties"]:checked')
  ).map((cb) => parseInt(cb.value));

  const selectedSkills = Array.from(
    document.querySelectorAll('#selected-skills .skill-item')
  ).map((item) => ({
    id: parseInt(item.dataset.id),
    naam: item.textContent.replace('×', '').trim(),
  }));

  const selectedLanguages = Array.from(
    document.querySelectorAll('#selected-languages .skill-item')
  ).map((item) => ({
    id: parseInt(item.dataset.id),
    naam: item.textContent.replace('×', '').trim(),
  }));
...
function setupAutoSave(bedrijfId) {
  // Save criteria when checkboxes change
  document
    .querySelectorAll('input[name="bedrijfFuncties"]')
    .forEach((checkbox) => {
      checkbox.addEventListener('change', () => {
        const criteria = collectCurrentCriteria(bedrijfId);
        saveSearchCriteria(bedrijfId, criteria);
      });
    });

  // Save criteria when skills/languages are added or removed
...
export async function renderSearchCriteriaBedrijf(
  ...
  // Laad functies uit API
  alleBeschikbareFuncties = await fetchAlleFuncties();
  ...
  // Dynamische werktype ID mapping gebaseerd op API functies
  const werktypeMapping = {};
  const werktypeReverseMapping = {};

  // Bouw mappings op basis van de geladen functies
  alleBeschikbareFuncties.forEach((functie) => {
    const nameLower = functie.naam.toLowerCase();
    werktypeMapping[nameLower] = functie.id;
    werktypeReverseMapping[functie.id] = nameLower;
  });
  ...
  async function saveSkillToDatabase(skillId) {
    ...
    try {
      ...
      // Update opgeslagen criteria na skill-toevoeging
      setTimeout(async () => {
        const criteria = collectCurrentCriteria(bedrijfId);
        saveSearchCriteria(bedrijfId, criteria);
      }, 100);
  ...
  async function removeSkillFromDatabase(skillId) {
    try {
      ...
      // Update opgeslagen criteria na skill-verwijdering
      setTimeout(async () => {
        const criteria = collectCurrentCriteria(bedrijfId);
        saveSearchCriteria(bedrijfId, criteria);
      }, 100);
  ...
  async function loadBedrijfWerktypes() {
    ...
    // Probeer eerst opgeslagen criteria te laden
    const savedCriteria = loadSearchCriteria(bedrijfId);

    if (savedCriteria && savedCriteria.functies) {
      // Gebruik opgeslagen functies
      savedCriteria.functies.forEach((functieId) => {
        const checkbox = document.querySelector(
          `input[name="bedrijfFuncties"][value="${functieId}"]`
        );
        if (checkbox) checkbox.checked = true;
      });
      console.log('Opgeslagen bedrijf zoek-criteria geladen:', savedCriteria);
      return;
    }

    // Fallback: haal huidige functies op uit API
    try {
      const response = await makeAuthenticatedRequest(
        `https://api.ehb-match.me/bedrijven/${bedrijfId}/functies`,
        { method: 'GET' }
  ...
  Promise.allSettled([
    ...
    // Setup auto-save voor zoek-criteria
    const bedrijfId = await getCurrentBedrijfId();
    if (bedrijfId) {
      setupAutoSave(bedrijfId);
    }
  });
