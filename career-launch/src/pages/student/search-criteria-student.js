...
async function syncStudentSkills(studentId, checkedSkillIds, checkedTaalIds, allStudentSkillIds, allStudentTaalIds) {
    ...
    const radiobuttonsHtml = functies
      .map(f => `
        <label class="checkbox-option">
          <input type="radio" name="jobType" value="${f.id}" data-type="${f.type}" ${studentData.zoekType == String(f.id) ? 'checked' : ''} ${readonlyMode ? 'disabled' : ''}>
          <span>${f.naam}</span>
        </label>
      `).join('');
    ...
    function renderCheckboxes(list, checkedArr, name) {
      return list
        .map(
          (item) => `
            <label class="checkbox-option">
              <input type="checkbox" name="${name}" value="${item.id}" ${isChecked(item.id, checkedArr) ? "checked" : ""} ${readonlyMode ? "disabled" : ""}>
              <span>${item.naam}</span>
            </label>
          `
        )
    ...
    if (form) {
      ...
      if (resetBtn) {
        resetBtn.addEventListener("click", async () => {
          ...
          let allStudentSkillIds = studentSkillsAll.filter(s => s.type === 0).map(s => s.id);
          let allStudentTaalIds = studentSkillsAll.filter(s => s.type === 1).map(s => s.id);
      ...
      if (saveBtn) {
        form.addEventListener("submit", async (e) => {
          e.preventDefault();
          let checkedSkills = Array.from(form.querySelectorAll('input[name="skills"]:checked')).map(i => Number(i.value));
          let checkedTalen = Array.from(form.querySelectorAll('input[name="talen"]:checked')).map(i => Number(i.value));
          ...
          if (skillInput && skillInput.value.trim()) {
            ...
            if (!allSkills.some(skill => skill.naam.toLowerCase() === value.toLowerCase())) {
              try {
          ...
          if (taalInput && taalInput.value.trim()) {
            ...
            if (!allTalen.some(taal => taal.naam.toLowerCase() === value.toLowerCase())) {
              try {
          ...
          let allStudentSkillIds = studentSkillsAll.filter(s => s.type === 0).map(s => s.id);
          let allStudentTaalIds = studentSkillsAll.filter(s => s.type === 1).map(s => s.id);
          await syncStudentSkills(studentId, checkedSkills, checkedTalen, allStudentSkillIds, allStudentTaalIds);
          try {
            await saveCriteriaToApi(studentId, {
      ...
      if (addSkillBtn) {
        addSkillBtn.addEventListener("click", async () => {
          ...
          try {
            ...
            const checkedSkills = Array.from(document.querySelectorAll('input[name="skills"]:checked')).map(i => Number(i.value));
            // Voeg de nieuwe toe aan de selectie:
            if (!checkedSkills.includes(nieuwSkill.id)) checkedSkills.push(nieuwSkill.id);
      ...
      if (addTaalBtn) {
        addTaalBtn.addEventListener("click", async () => {
          ...
          try {
            ...
            // Haal alle nu geselecteerde vinkjes op:
            const checkedTalen = Array.from(document.querySelectorAll('input[name="talen"]:checked')).map(i => Number(i.value));
            // Voeg de nieuwe toe aan de selectie:
            if (!checkedTalen.includes(nieuweTaal.id)) checkedTalen.push(nieuweTaal.id);
            // Zet als geselecteerd voor render:
            studentData.talen = checkedTalen;
