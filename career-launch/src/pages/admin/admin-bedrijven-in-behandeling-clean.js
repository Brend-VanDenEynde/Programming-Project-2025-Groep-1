// filepath: c:\Users\larsc\Documents\Programming-Project-2025-Groep-1\career-launch\src\pages\admin\admin-bedrijven-in-behandeling.js
...
export async function renderAdminBedrijvenInBehandeling(rootElement) {
  ...
  `;
  // Fetch unapproved companies from API using new API utilities
  try {
  ...
  // Handle approve buttons with new API utilities
    ...
    btn.addEventListener('click', async () => {
      ...
      if (confirm(`Weet je zeker dat je ${companyName} wilt goedkeuren?`)) {
        try {
          ...
          btn.disabled = true;
          btn.textContent = '...'; // Call the API to approve the company using new API utilities
  ...
  // Handle reject buttons with new API utilities
    ...
    btn.addEventListener('click', async () => {
      ...
      if (
        ...
        try {
          // Disable the button to prevent double-clicks
          btn.disabled = true;
          btn.textContent = '...'; // Call the DELETE user endpoint to reject/delete the company using new API utilities
